...
- **Tests are immutable** — existing tests may only be modified when (a) the corresponding
  spec changed in the same PR, or (b) the PR body's "Test changes" section gives a fachliche
  reason. Modifying tests to make them pass is forbidden. Adding new tests is always allowed.
